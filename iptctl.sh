...
# 加载国际化函数库（仅 source，不立即初始化；等 config_load 后再初始化语言）
IPTCTL_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
I18N_FUNCTIONS="$IPTCTL_DIR/i18n/i18n_functions.sh"
I18N_AVAILABLE=0

if [[ -f "$I18N_FUNCTIONS" ]]; then
    source "$I18N_FUNCTIONS"
    I18N_AVAILABLE=1
else
    # 定义简单的回退函数（i18n 库不存在时使用）
    t() {
        echo "$1"
    }
...
i18n_init_after_config() {
    if [[ "$I18N_AVAILABLE" -eq 1 ]]; then
        if ! init_i18n; then
            echo "警告: 国际化初始化失败，继续使用默认语言" >&2
        fi
    else
        echo "警告: 国际化函数库未找到，使用回退函数" >&2
    fi
}
...
bi() {
    ...
    # 翻译键模式：小写字母、数字、下划线，中间用点号分隔（如 ui.welcome）
    if [[ "$msg" =~ ^[a-z][a-z0-9_]*(\.[a-z0-9_]+)+$ ]]; then
        local translated
        translated="$(t "$msg")"
        # 只有翻译结果与键不同时，才认为翻译成功
        [[ "$translated" != "$msg" ]] && out="$translated"
    fi
    
    ui_translate_line "$out"
}
...
select_mode() {
  while true; do
    bi ""
    bi "╔════════════════════════════════════════════════════════════╗"
    bi "║                 iptctl · $(t "ui.welcome")                 ║"
    bi "╚════════════════════════════════════════════════════════════╝"
    bi "ui.select_mode"
    bi ""
    bi "  1) $(t "ui.beginner_mode")"
    bi "     $(t "modes.beginner_description")"
    bi ""
    bi "  2) $(t "ui.standard_mode")"
    bi "     $(t "modes.standard_description")"
    bi ""
    bi "  3) $(t "ui.expert_mode")"
    bi "     $(t "modes.expert_description")"
    bi ""
    bi "  0) $(t "ui.exit")"
    bi ""
    local c
    c="$(read_num "$(t "ui.select_mode"): ")"
    case "$c" in
      0) iptctl_exit 0 ;;
...
main() {
  config_load
  # config_load 完成后再初始化 i18n，确保 IPTCTL_LANG 已从配置文件读取
  i18n_init_after_config
  ui_style_maybe_prompt
  while true; do
    select_mode
